...
public class AirNPCCommand implements CommandExecutor {

    private final ATLATestQuest plugin;

    public AirNPCCommand(ATLATestQuest plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(sender instanceof Player){
            ...
            //2. save the location
            plugin.getConfig().set("airnpclocation", location);
            plugin.saveConfig();
            player.sendMessage("Air NPC Location set!");
